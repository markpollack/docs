...
- Keep Claude Code, Codex, and Gemini CLI as the actively verified provider set: both new CLIs authenticate interactively and cache credentials, so neither can be wired into the parity workflow's secret-based job pattern
