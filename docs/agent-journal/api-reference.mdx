---
title: "API Reference"
description: "Agent Journal 1.10.1 core, storage, event, evaluation, and capture APIs"
---

This page describes the public API shipped in Agent Journal 1.10.1.

## Coordinates

| Artifact | Purpose | Java |
|---|---|---|
| `io.github.markpollack:journal-core:1.10.1` | Core domain, storage, evaluation, feedback, and traces | 17+ |
| `io.github.markpollack:claude-code-capture:1.10.1` | Claude Code capture | 21+ runtime |
| `io.github.markpollack:gemini-cli-capture:1.10.1` | Gemini CLI capture | 21+ runtime |
| `io.github.markpollack:grok-cli-capture:1.10.1` | Grok CLI capture | 17+ |
| `io.github.markpollack:codex-cli-capture:1.10.1` | Codex CLI capture | 17+ |
| `io.github.markpollack:antigravity-cli-capture:1.10.1` | Antigravity CLI capture | 17+ |
...
| `repository(String)` | Accepts a repository path; 1.10.1 does not persist it or act on it |
...
There is no `config(Map)` overload in 1.10.1.
The `repository(String)` setter is present, but the 1.10.1 run implementation does not consume the stored builder value.
...
There is no five-argument `TokenUsage.of(...)` overload in 1.10.1.
...
- `JsonFileStorage` does not validate path containment for caller-supplied identifiers or artifact names in 1.10.1.
