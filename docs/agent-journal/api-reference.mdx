---
title: "API Reference"
description: "Agent Journal 1.6.0 core, storage, event, evaluation, and capture APIs"
---
...
|---|---|---|
| `io.github.markpollack:journal-core:1.6.0` | Core domain, storage, evaluation, feedback, and traces | 17+ |
| `io.github.markpollack:claude-code-capture:1.6.0` | Claude Code capture | 21+ runtime |
| `io.github.markpollack:gemini-cli-capture:1.6.0` | Gemini CLI capture | 21+ runtime |
...
| `repository(String)` | Accepts a repository path; 1.6.0 does not persist it or act on it |
...
There is no `config(Map)` overload in 1.6.0.
The `repository(String)` setter is present, but the 1.6.0 run implementation does not consume the stored builder value.
...
There is no five-argument `TokenUsage.of(...)` overload in 1.6.0.
