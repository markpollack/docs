...
The generated aggregate Javadoc remains the detailed signature reference, and the [tutorial source](https://github.com/markpollack/agent-judge-tutorial/tree/106ca35a74ebdd9995329cb0bde63d66a9e7e8ed) supplies compiled usage examples.
...
| Component | Contract |
|---|---|
| `status` | Required `PASS`, `FAIL`, `ABSTAIN`, `NOT_APPLICABLE`, or `ERROR` |
| `score` | Optional finite `Double` normalized to `[0.0, 1.0]` |
| `label` | Optional non-blank classification |
| `reasonCode` | Optional; required on `ERROR` from the instrument family, optional on `FAIL` from the subject family, forbidden otherwise |
| `reasoning` | Required text; non-blank for `ABSTAIN`, `NOT_APPLICABLE`, and `ERROR`, and for a coded `FAIL` |
| `checks` | Immutable granular Boolean checks |
| `metadata` | Recursively portable and immutable evidence |

Direct factories are `pass(reasoning)`, `fail(reasoning)`, `abstain(reasoning)`, `notApplicable(reasoning)`, and `error(reasoning)`.
Returning `NOT_APPLICABLE` requires an [exclusion condition declared in advance](/docs/agent-judge/migration-0.17#exclusion-must-be-declared-in-advance).
...
`effectiveScore()` returns the explicit score when present, derives `1.0` or `0.0` for an unscored `PASS` or `FAIL`, and is empty for `ABSTAIN`, `NOT_APPLICABLE`, or `ERROR`.
...
Voting strategies are `MajorityVotingStrategy`, `ConsensusStrategy`, `AverageVotingStrategy`, `WeightedAverageStrategy`, `MedianVotingStrategy`, `ConjunctiveStrategy`, and `AllMustPassStrategy`.
`TiePolicy`, `ErrorPolicy`, `TierPolicy`, and `NotApplicablePolicy` make edge behavior explicit.

`Verdict` carries seven components in 0.17, including `seats` and `decision`. See [Migrating to 0.17](/docs/agent-judge/migration-0.17) and [Interpreting verdicts](/docs/agent-judge/interpreting-verdicts).
...
Use all Agent Judge artifacts at version `0.17.0`.
