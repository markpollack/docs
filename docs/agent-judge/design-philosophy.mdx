...
agent-judge-agent-client      (+ AgentClient, provided)
```
...
3. **Evaluator** — a static convenience class with 4 methods (Judge/Jury x with/without extra metadata) that combines execution + context building + evaluation into a one-liner.
