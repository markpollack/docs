...
`pass_rate` returns absent rather than `0` when no items qualify. Describe that as
**no items qualify for inclusion in this pass rate** — never as "no item was judged", which is a
different claim and is usually false in exactly this case.

Two absences leave the scoring set for different reasons, and they stay distinct:
...
An item can also be `NOT_APPLICABLE` — excluded and counted separately. So there can be nothing
left to score even though every item was judged.

## What this deliberately does not say

Whether any reading counts against the subject, counts toward a rate, or changes that rate. **That
policy belongs to the consumer that owns the counting**, which is why the counting above is
