...
Five statuses now. The difference between the last three is how each status is counted.

| Status | The question | How it is counted |
|---|---|---|
| `PASS` | asked, answered yes | counted as a success |
...
| `ERROR` | never reached | not counted against the subject |

`ABSTAIN` no longer means "not applicable" anywhere. Missing evidence is still `ABSTAIN`.

### Exclusion must be declared in advance

Exclusion is the one outcome that lets a judge exclude its own judgment from the combined result,
which makes it the one outcome an instrument could use to dodge a question it does not like the look of.
...
independently. The roster is what the result is measured against. With nothing in it, nothing could
fail, so the roll-up passed.
