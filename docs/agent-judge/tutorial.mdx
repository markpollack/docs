...
description: "Step by step — from a single judge to a multi-judge jury that grades agent output"
...
- The `agent-judge-core` and `agent-judge-exec` dependencies ([setup](/docs/agent-judge/getting-started#add-the-dependency))
...
// Three judges with different weights
...
This is the core pattern: **cheap judges run first and filter; expensive judges confirm**.
In production, you can formalize this with a `CascadedJury` that stops evaluation early when cheap judges already have a verdict.
