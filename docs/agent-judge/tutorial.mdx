...
Bridge evaluators either run a framework call or adapt an existing response into a `JudgmentContext`. The judges and jury don't change — swap `KoogEvaluator` for `SpringAiEvaluator`, `LangChain4jEvaluator`, or `AgentClientEvaluator`.
