...
one, naming them. An excluded criterion drops out of what the result is measured against, and is
reported on its own, with a mandatory reason. Excluding an unconditional criterion, or excluding one with no reason, is a
...
The roster is what the result is measured against. With nothing in it, nothing could fail, so the
roll-up passed. Released 0.16.0 returns `PASS` for that, and the result is indistinguishable in every
stored field from a specification that was genuinely met.
...
should not have been asked, left out of the pass rate, and **counted separately**. Exclusion must be
