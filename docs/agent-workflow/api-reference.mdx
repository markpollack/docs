...
The `StepRunner` interface is the seam for portable runtimes. Today only `LocalStepRunner` is implemented. Planned:

| Runner | What it adds | Status |
|--------|-------------|--------|
| `LocalStepRunner` | Direct in-process execution, zero overhead | Available |
| `CheckpointingStepRunner` | JDBC crash recovery via `workflow-batch` | Stage 3.0 |
| `TemporalStepRunner` | Distributed durable execution | Stage 3.1 |

Same workflow code — swap the `@Bean`, not the workflow.
...
├── workflow-batch/      # StepRunner implementations (planned)
