...
@Bean PartitionHandler partitionHandler() {
    return new LocalPartitionHandler();                            // default
    // return new CheckpointingPartitionHandler(dataSource);       // JDBC
    // return new TemporalPartitionHandler(client, "task-queue");  // Temporal
}
```

| Handler | What it adds | Requires |
|---------|-------------|----------|
| `LocalPartitionHandler` | Nothing — in-process, zero overhead | Nothing |
| `CheckpointingPartitionHandler` | JDBC crash recovery (Spring Integration MessageStore) | A `DataSource` |
| `TemporalPartitionHandler` | Distributed durable execution | Temporal Server |

Graduation path: Local → Checkpointing → Temporal. Each a single `@Bean` swap.
...
├── workflow-batch/      # CheckpointingPartitionHandler (JDBC + Spring Integration)
├── workflow-temporal/   # TemporalPartitionHandler (optional Temporal SDK)
