...
description: "Compose steps into workflows with typed context, crash recovery, and quality gates"
---

## What is Agent Workflow?

A workflow is a sequence of **steps**. Each step does one thing — calls an LLM, runs a function, invokes an external agent. Steps pass data through a shared **context** (typed key-value pairs). The workflow compiles to a **graph IR** that can execute locally, with JDBC crash recovery, or on Temporal — same code, different runtime.
...
| `A2AStep` | Remote agent via Agent Communication Protocol | Minutes |
...
- **Crash recovery** — completed steps are checkpointed; on restart, execution resumes from where it left off
...
If your app crashes halfway through a workflow, you don't want to re-run completed steps (especially $5 LLM calls). Swap the runtime with a single `@Bean`:

```java
@Bean PartitionHandler partitionHandler(DataSource ds) {
    return new CheckpointingPartitionHandler(ds);  // JDBC crash recovery
}
```

On restart, the executor reads the checkpoint, skips completed steps, and resumes from the last unfinished step. Same workflow code — zero changes.

**Graduation path**: start with `LocalPartitionHandler` (zero overhead), add `CheckpointingPartitionHandler` when you need crash recovery, graduate to `TemporalPartitionHandler` for distributed durability.
