...
| `JobRepository` (JDBC) | Planned (JDBC `StepRunner`) | Crash recovery — resume from last completed step (not yet implemented) |
...
### JobRepository → StepRunner (planned)

**Spring Batch**: `JobRepository` persists step execution state to JDBC. On restart, completed steps are skipped.

**Agent Workflow**: The `StepRunner` interface is the seam for future crash recovery. Today only `LocalStepRunner` exists (in-process, zero overhead). JDBC-backed checkpointing is planned — the same pattern as Spring Batch's `JobRepository`, but for steps that cost $5 each instead of milliseconds.
...
| **Crash recovery** | JobRepository (always on) | StepRunner seam (planned) |
