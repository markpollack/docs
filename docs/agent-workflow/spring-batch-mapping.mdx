...
| `JobRepository` (JDBC) | Planned (JDBC `PartitionHandler`) | Crash recovery — resume from last completed step (not yet implemented) |
...
### JobRepository → PartitionHandler (planned)

**Spring Batch**: `JobRepository` persists step execution state to JDBC. On restart, completed steps are skipped.

**Agent Workflow**: The `PartitionHandler` interface is the seam for future crash recovery. Today only `LocalPartitionHandler` exists (in-process, zero overhead). JDBC-backed checkpointing is planned — the same pattern as Spring Batch's `JobRepository`, but for steps that cost $5 each instead of milliseconds.
...
| **Crash recovery** | JobRepository (always on) | PartitionHandler seam (planned) |
