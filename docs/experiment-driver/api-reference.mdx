...
Runtime candidates still return Agent Judge 0.14 `Judgment` values. Agent Experiment converts them
immediately to its normalized `RecordedJudgment` persistence boundary. Existing 0.5 / Agent Judge 0.13
result files are upgraded on read; saving them writes the normalized 0.6 format.
...
<!-- Agent Workflow integration (typed workflow invokers and journal adaptation) -->
<dependency>
    <groupId>io.github.markpollack</groupId>
    <artifactId>experiment-workflow</artifactId>
    <version>0.6.0</version>
</dependency>
```
