...
Review the staged changes (`git diff --cached`). For each file:

1. Check for bugs, security issues, and performance problems
2. Verify test coverage for new code paths
3. Flag any style violations

Summarize findings as a table. $ARGUMENTS
```

### How it works

1. **YAML front matter** — `name` and `description` fields. If no front matter, the filename becomes the name.
