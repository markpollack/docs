...
<Note>
  Each release publishes two jars. The one you run is **`loopy-<version>-exec.jar`** — the
  ...
  and is not runnable with `java -jar`.
</Note>

<Steps>
  <Step title="Download the executable jar">
    ```bash
    curl -LO https://github.com/markpollack/loopy/releases/latest/download/loopy-0.5.0-exec.jar
    ```
  </Step>
  <Step title="Check the download">
    ```bash
    java -jar loopy-0.5.0-exec.jar --version
    ```
    `--version` and `--help` work without an API key, so you can confirm the download
    before setting up a provider.
  </Step>
  <Step title="Run it">
    ```bash
    java -jar loopy-0.5.0-exec.jar
    ```
...
The launcher finds the jar, checks for Java, and applies the JVM flags Loopy expects:

```bash
curl -LO https://github.com/markpollack/loopy/releases/latest/download/loopy-0.5.0-exec.jar
curl -LO https://raw.githubusercontent.com/markpollack/loopy/main/loopy
chmod +x loopy
./loopy
```

Keep the script beside the jar, or set `LOOPY_JAR` to the jar's path. Windows users want
