...
through the corrected library did not reproduce it. The variant
...
against the 30–90% seen in v3 and v4. **Updated 2026-09-15 — the relative ordering is withdrawn too.** Reanalysis found the arms do not
share a start state, so a start-state defect does not cancel between them, and no corrected figure
is offered while that work is open. At nine runs across nine variants, three of them a single run,
v1 cannot carry a comparison in any case. Kept as a record of method, not of result.
</Note>
