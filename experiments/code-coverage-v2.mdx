...
*The Markov fundamental matrix predicts how many times the agent cycles through each loop before absorbing. Skills+preanalysis roughly halves the EXPLORE loop — same attention budget, better allocation.*
...
**The halving is real; the magnitudes were not.** Both arms were inflated by the same factor, so the
ratio survived while the numbers did not.
</Warning>
