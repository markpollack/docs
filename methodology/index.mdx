...
Six pillars — the thesis, the development methodology, knowledge design, execution structure, evaluation, and behavioral analysis — plus the improvement flywheel that ties them into a single loop:
...
<CardGroup cols={3}>
  ...
  >
    Six phases, five variants, and the slash commands that run them
  </Card>
  <Card
    title="KB Design"
    icon="folder-tree"
...
| **Steps to completion** | Efficiency — **counted per run**, not modelled | Agent Journal |
...
<Warning>
**Updated 2026-08-27 — "Expected Steps" was removed from this table.** It read *"Efficiency (Markov
fundamental matrix)"* and recommended a modelled quantity where a counted one exists.

At unit cost, expected-steps-to-absorption from the run's starting state **equals the mean number of
steps in the run** — it is the transition matrix reduced to one number, and that number is one you
can simply count. Every published figure derived that way in this programme turned out to be wrong;
every counted figure held. The modelled version adds no information and inherits every defect in the
chain that produced it.

**Count the steps. Use the matrix for [localisation](/methodology/markov-fingerprinting) — *where*
the time went — which is the thing a count cannot tell you.**
</Warning>
