...
**Version 0.27.0** — [Maven Central](https://central.sonatype.com/search?q=g:io.github.markpollack+a:agent-client-core)
...
Claude Code, Codex, and Gemini CLI are actively verified in CI. Amazon Q, Amp, Qwen Code, and SWE-agent remain experimental adapters rather than current support claims; Grok and Antigravity join that experimental set in 0.28.0. Neither new CLI can authenticate non-interactively, so neither can be wired into the parity workflow, and neither enters the verified set. Provider selection happens at construction time — everything after `AgentClient.create(model)` uses the shared client contract.
