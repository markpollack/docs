...
**Version 0.31.0** — [Maven Central](https://central.sonatype.com/search?q=g:io.github.markpollack+a:agent-client-core)

**0.31.0:** `ClaudeAgentModel.interrupt()` closes in-flight clients across calls, iteration, and streaming; `close()` delegates to it. Built on Claude Agent SDK 1.7.0, which destroys the CLI process tree before closing streams. The `AgentModel` interface is unchanged. [Release notes](https://github.com/markpollack/agent-client/releases/tag/v0.31.0).
...
Since Agent Client **0.30.0**, the `agent-junie` provider brings Junie through the same Java API.
It speaks the Agent Client Protocol using `acp-core` as its transport, so it needs no provider SDK.
See the [0.30.0 release notes](https://github.com/markpollack/agent-client/blob/v0.30.0/RELEASE_NOTES_0.30.0.md).
