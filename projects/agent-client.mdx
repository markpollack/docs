...
**Version 0.26.0** — [Maven Central](https://central.sonatype.com/search?q=g:io.github.markpollack+a:agent-client-core)

**0.26.0:** Breaking diligence cleanup. Removes the abandoned Vendir context advisor and Git-repository DSL, retires the stale `agents-runtime` container build and Docker TCK, upgrades the dependency-security baseline, and pins release automation. Claude Code, Codex, and Gemini CLI are the actively verified provider set; other adapters remain experimental.
...
Claude Code, Codex, and Gemini CLI are actively verified in CI. Amazon Q, Amp, Qwen Code, and SWE-agent remain experimental adapters rather than current support claims. Provider selection happens at construction time — everything after `AgentClient.create(model)` uses the shared client contract.
