...
**Current release: 0.9.0** · Java 21 · three modules
...
|--------|-------------|------------------|
| `experiment-core` | Datasets, workspace provisioning, runner, journals, comparison, results, storage | Agent Judge 0.17.0, Agent Journal 1.10.0, Jackson |
| `experiment-claude` | Claude SDK invoker, plan generator, semantic judge | Claude Code SDK 1.6.0 |
| `experiment-workflow` | Typed workflow invokers with journal and cost adaptation | Agent Workflow 0.12.3, Agent Client 0.30.0 |
...
Version 0.9.0 records Agent Judge's verdict interpretation beside each result and uses Workflow 0.12.3. Earlier results remain readable and are not rewritten; `InterpretationReExport` can add readings while preserving stored verdicts. [Release notes](https://github.com/markpollack/agent-experiment/releases/tag/v0.9.0).
...
Standalone consumers resolve Agent Journal/Capture 1.10.0, Jackson 2.22.2, and Jackson 3.2.2 without
