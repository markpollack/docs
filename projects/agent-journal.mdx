...
|--------|-------------|--------------|
| `journal-core` | Events, storage, EvalSubject, Feedback, portable `TraceWriter` | Jackson 2, SLF4J API |
...
Agent Workflow's `workflow-journal` module integrates Agent Workflow with Agent Journal; it is not an Agent Journal module. Its `WorkflowJournal` adapter records each workflow step completion as a `WorkflowStepEvent` containing the step name, duration, tokens, and cost.
...
├── events.jsonl      # immutable execution events
├── analysis.jsonl    # regenerable derived events
