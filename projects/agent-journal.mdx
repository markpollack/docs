...
**What's new in 1.5.0:** First-class **Gemini CLI capture** — the new `gemini-cli-capture` module projects Gemini runs into the *same* portable trace + cost schema as Claude, so one analysis layer reads both runtimes — plus per-step cost attribution. Builds on 1.4.0's TraceWriter v2 content capture (`TraceContentMode` FULL / TRUNCATED / LENGTHS). Now **three modules**: `journal-core`, `claude-code-capture`, and `gemini-cli-capture`.
