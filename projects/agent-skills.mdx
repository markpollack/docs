...
- ~~Reduced expected steps by **~25%** compared to baseline~~ *(withdrawn — see below)*

<Warning>
**Updated 2026-09-15 — the ~25% is withdrawn, not corrected.** The 2026-08-27 note withdrew the
absolute figures but kept this ratio, on the argument that both arms were inflated by the same
factor (defect **CT6**). Reanalysis has since found a second defect that distorts each arm by where
its runs began, and these arms did not begin alike — so no common factor cancels, and the argument
holding the ratio up does not hold.

**No replacement figure is offered**, because that reanalysis is not settled. The page also never
said which baseline arm the comparison used, which is a second reason it cannot be repaired as
written. The v2 data is kept as a record of method, not of result. See the
[v2 experiment page](/experiments/code-coverage-v2).
</Warning>
