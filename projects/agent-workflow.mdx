...
The biggest insight from running real agent experiments: the AI shouldn't do everything. A PR merge workflow illustrates this:

```
fetchPR (deterministic) → analyzeDiff (LLM) → checkConflicts (deterministic)
    → resolveConflicts (LLM) → runTests (deterministic) → merge (deterministic)
```

Five of six steps are deterministic — GitHub API calls, git operations, build commands. Only two need LLM reasoning: analyzing the diff and resolving conflicts. The deterministic steps are free, fast, and perfectly reliable. The LLM steps are expensive and variable. By minimizing what the LLM needs to do, you reduce cost, increase reliability, and make the whole pipeline easier to debug.
