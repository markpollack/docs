...
>
> **What's new in 0.12.3:** Compatibility with Agent Judge 0.17.0. `JudgeGate` and `TieredGate` treat a `NOT_APPLICABLE` aggregate as inconclusive, preserve the verdict, and route it through the existing error handler. [Release notes](https://github.com/markpollack/agent-workflow/releases/tag/v0.12.3).
>
...
## Hosted execution with ManagedAgentStep

`ManagedAgentStep` runs a workflow step in Anthropic's hosted sandbox, with bash, file operations, and web tools.
The workflow graph still controls the sequence; the hosted agent performs the work inside that step and returns its text result.
Each execution creates a new session for the agent.

Use `workflow-flows`, set `ANTHROPIC_API_KEY` for an account with Managed Agents access, and create an agent and environment in Anthropic's API first.
In this example from the [0.7.0 release notes](/docs/agent-workflow/whats-new#managedagentstep), `agentId` and `envId` are those existing IDs, and `analyzeStep` and `validateStep` are steps you have already defined:
...
Add the official Anthropic SDK dependency alongside `workflow-flows`.
It is optional and needed only when you use `ManagedAgentStep`:
...
The [ManagedAgentStep release notes](/docs/agent-workflow/whats-new#managedagentstep) also show timeouts, direct execution, and provisioning an agent inline with `ManagedAgentStep.create(...)`.
...
A real [PR merge workflow](https://github.com/markpollack/agentworks-pr-review) illustrates the point. The pipeline has 8 steps:
...
<CardGroup cols={2}>
  <Card title="Source Code" icon="github" href="https://github.com/markpollack/agent-workflow">
    Source code (0.12.3 on Maven Central)
  </Card>
