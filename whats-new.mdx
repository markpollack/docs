...
- `NOT_APPLICABLE` is the fifth judgment status — a question that should not have been asked, left out of the pass rate, and **counted separately**. Exclusion must be declared in advance through `JudgeMetadata.notApplicableWhen`, and `NotApplicablePolicy` (`REFUSE` by default) governs what a strategy does with one
...
- The EARS and RFC 2119 requirements judges gain conditional criteria, and reject an empty roster. In 0.16.0, with nothing to check, nothing could fail, so it passed
